...
type Props = { isOverlay: boolean; onRecordRating: (results: RatingResult[]) => void; onShowDraft: () => void; onShowRating: () => void; onShowTeams: () => void; onShowKing: () => void; onShowRoster: () => void };
...
const initial: Match[] = [
  ["upper-1", "Player 01", "Player 02"], ["upper-2", "Player 03", "Player 04"], ["upper-final", "", ""],
  ["lower-1", "Player 05", "Player 06"], ["lower-2", "Player 07", "Player 08"], ["lower-3", "", ""], ["lower-4", "", ""], ["lower-5", "", ""], ["lower-6", "", ""], ["grand-final", "", ""],
...
function read(): State { try { const stored = JSON.parse(localStorage.getItem(storageKey) ?? "null") as State | null; if (stored?.matches?.length === initial.length) return { ...stored, matches: stored.matches.map((match) => ({ ...match, players: match.players.map((name) => name.replace(/^Игрок (\d+)$/, "Player $1")) as Match["players"] })), tournamentId: stored.tournamentId || newId() }; } catch { /* stale data is replaced */ } return fresh(); }
...
function Card({ match, disabled, editable: canEdit, onChange }: { match: Match; disabled: boolean; editable: boolean; onChange: (id: string, row: 0 | 1, field: "player" | "score", value: string) => void }) {
  ...
  return <div className="bracket-match">{([0, 1] as const).map((row) => <div className="bracket-match__row" key={row}><input aria-label={`Player nickname ${row + 1}`} disabled={disabled || !canEdit} placeholder="Player nickname" value={match.players[row]} onChange={(event) => onChange(match.id, row, "player", event.target.value)} /><input aria-label={`Player score ${row + 1}`} className="bracket-match__score" disabled={!canScore} inputMode="numeric" maxLength={2} placeholder="–" value={match.scores[row]} onChange={(event) => onChange(match.id, row, "score", event.target.value.replace(/[^0-9]/g, ""))} /></div>)}</div>;
}

export function BracketBoard({ isOverlay, onRecordRating, onShowDraft, onShowRating, onShowTeams, onShowKing, onShowRoster }: Props) {
